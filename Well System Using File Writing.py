...
def wellDistance():
    global pump_x, pump_y, current_x, current_y, well_num, k, d, s, t, q, total_drawdown, total_flow, diff_k, diff_q

    new_k=k
    new_q=q

    if well_num in diff_k:
        new_k=float(input('\nEnter aquifer permeability in m/s (k value) for well {}: '.format(well_num)))

    if well_num in diff_q:
        new_q=float(input('\nEnter well flow rate in m^3/s (q value) for well {}: '.format(well_num)))

    distance=((pump_x-current_x)**2+(pump_y-current_y)**2)**0.5
    calc1=(-0.5772)-(log(((distance**2)*s)/(4*new_k*d*t)))
    calc2=new_q/(4*pi*new_k*d)
...
k=float(input('Enter aquifer permeability in m/s (k value): '))
d=float(input('Enter original aquifer saturated thickness in m (D value): '))
s=float(input('Enter aquifer storage coefficient (S value): '))
t=float(input('Enter time since pumping began in s (t value): '))

diff_k=input('\nEnter well number for different k values (space in between): ').split()
diff_k=[int(i) for i in diff_k]

diff_q=input('Enter well number for different q values (space in between): ').split()
diff_q=[int(i) for i in diff_q]
...
txt_file.write('Aquifer permeability in m/s (k value): {}\n'.format(k))
txt_file.write('Original aquifer saturated thickness in m (D value): {}\n'.format(d))
